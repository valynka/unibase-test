export default class SortableTable {
  ...
  onSortClick = (event) => {
    ...
    if (!clickedColumn.dataset.order) {
      ...
      this.sorted.order =
        clickedColumn.dataset.order === "asc" ? "desc" : "asc";
  ...
  onFilter = (event) => {
    ...
    if (substr.length > 3) {
      this.filterData(arrData, substr);
    }
  };
  ...
  getSearchInput() {
    return `<div class="sortable-table__search">
             <input data-elem="search" type="text" class="sortable-table__search-input">
            </div>`;
  }
  ...
  getTableBody(data) {
    this.dataFields = this.headersConfig.map(({ id, template }) => {
      return { id, template };
    });
  ...
  initEventListeners() {
    ...
    this.subElements.search.addEventListener("input", this.onFilter);
  }
  ...
  sort({ id, order }) {
    ...
    allColumns.forEach((column) => {
      if (column.dataset.id !== "images") column.dataset.order = "";
    });

    currentColumn.dataset.order = order;

    this.subElements.body.innerHTML = sortedData
      .map((item) => this.getTableRow(item))
      .join("");
  }
  ...
  filterData(arrData, substr) {
    const filteredData = arrData.filter((post) => {
      const title = post.title.toLowerCase();
      const body = post.body.toLowerCase();
      return title.includes(substr) || body.includes(substr);
    });
    this.subElements.body.innerHTML = filteredData
      .map((item) => this.getTableRow(item))
      .join("");
  }
